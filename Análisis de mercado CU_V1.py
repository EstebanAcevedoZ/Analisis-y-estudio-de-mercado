...
# Fecha: 2026
# Proyecto: Herramienta 1 de Análisis Inmobiliario por Proximidad usando Coordenada Urbana
# ==========================================
...
def procesar_con_fechas_y_mapa(archivo_path, lista_predios, lista_radios, col_agrupadora, operaciones, col_peso=None):
    print("Cargando datos de CU...")
    df_proyectos = pd.read_excel(archivo_path, sheet_name='Base General', skiprows=6)
    df_inmuebles = pd.read_excel(archivo_path, sheet_name='Base General', skiprows=6)

    # Convertir Fecha Inicio Ventas a datetime inmediatamente
    # df_proyectos['Fecha Inicio Ventas'] = pd.to_datetime(df_proyectos['Fecha Inicio Ventas'], dayfirst=True, errors='coerce')
    ...
    df_proyectos = df_proyectos.dropna(subset=['Latitud', 'Longitud'])
    
    col_vivienda_proyectos = "Segmento"
    resultados_filas = []
    ...
    for predio in lista_predios:
        ...
        def calcular_distancia(fila):
            try: return geodesic(coord_centro, (fila['Latitud'], fila['Longitud'])).km
            except: return float('inf')
            
        df_proyectos['Distancia_Actual'] = df_proyectos.apply(calcular_distancia, axis=1)

        for radio_km in lista_radios:
            mask_cercanos = (df_proyectos['Distancia_Actual'] <= radio_km)
            proyectos_cercanos = df_proyectos[mask_cercanos].copy()
            ids_proyectos = proyectos_cercanos['Id Proyecto'].unique()
            
            if radio_km == max(lista_radios):
                proyectos_para_mapa = pd.concat([proyectos_para_mapa, proyectos_cercanos]).drop_duplicates(subset=['Id Proyecto'])
            ...
            if not proyectos_cercanos.empty:
                ...
                #f_min = proyectos_cercanos['Fecha Inicio Ventas'].min()
                #f_max = proyectos_cercanos['Fecha Inicio Ventas'].max()
                ...
                # fila_datos[("Fecha Inicio Ventas", "Antigua")] = f_min
                # fila_datos[("Fecha Inicio Ventas", "Reciente")] = f_max
                ...
                # Métricas Inmuebles
                inmuebles_f = df_inmuebles[df_inmuebles['Id Proyecto'].isin(ids_proyectos)].copy()
    ...
    columnas_ordenadas = cols_base + sorted(list(todas_las_columnas)) + [("Fecha Inicio Ventas ", "Antigua"), ("Fecha Inicio Ventas", "Reciente")]
    ...
    nombre_excel = f"Analisis_Predio_{nombre_combinado}_cu.xlsx"
    ...
    fig.add_trace(go.Scattermap(
        lat=proyectos_para_mapa['Latitud'],
        lon=proyectos_para_mapa['Longitud'],
        mode='markers',
        marker=dict(size=8, color='blue', opacity=0.5),
        text=proyectos_para_mapa['Id Proyecto'],
    ...
    fig.write_html(f"Mapa_Predio_{nombre_combinado}_cu.html")
    fig.show()

# --- CONFIGURACIÓN ---
config = {
    "archivo_path": "Coordenada Urbana/Base General Cundinamarca May 2026.xlsx",
    "lista_predios": [
        {"nombre": "Sosiego", "lat": 4.574917958587793, "lon": -74.07973063953699},
        {"nombre": "San Blas", "lat": 4.5638026411122965, "lon": -74.08025810425437}
    ],
   
    "lista_radios": [0.5, 1.0, 1.5, 2.0, 2.5, 3.0], 
    "col_agrupadora": "Segmento",
    "col_peso": "Unidades Por Tipo", 
    "operaciones": {
        "Unidades Por Tipo": ["suma"],
        "Precio 2026-04-01": ["ponderado"],
        "Precio metro cuadrado 2026-04-01": ["ponderado"],
        "Área Por Tipo": ["ponderado"],
        "Alcobas": ["ponderado"],
        "Baños": ["ponderado"]
    }
}
